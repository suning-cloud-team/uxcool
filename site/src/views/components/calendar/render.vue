<template>
  <ux-demo :height="200"
           title="自定义渲染">
    <div slot="demo">
      <ux-calendar :month-cell-content-render="monthCellContentRender"
                   @change="onChange">
    ...
    <div slot="desc">
      使用
      <code>slot-scope=dateCellContentRender</code>,自定义渲染
    </div>
    <ux-code slot="code">
      {{ code }}
    </ux-code>
...
<script>
  import code from '@/code/calendar/render.vue';
      ...
      monthCellContentRender({ current }) {
        ...
        return (
          <ul>
            {list.map(item => (
              <li>
                <ux-badge status={item.type} text={item.msg} />
              </li>
            ))}
          </ul>
        );
