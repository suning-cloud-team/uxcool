    ...
    getVarAllHeight: function() {
      ...
      if (delta.total - delta.end <= delta.keeps || delta.varLastCalcIndex === delta.total - 1) {
        ...
        return (
          ...
          (delta.total - delta.end) * (delta.varAverSize || this.size)
        );
      }
    },
  ...
  render: function(h) {
    ...
    return h(
      this.rtag,
      {
        ...
        on: {
          '&scroll': dbc ? debounce(this.onScroll.bind(this), dbc) : this.onScroll,
        },
      },
      [
        h(
