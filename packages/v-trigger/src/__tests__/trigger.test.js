import { waitTime, mount, shallowMount, $ } from '@suning/v-test-utils';
...
const TestNestTrigger = {
  ...
  mounted() {
    const { $refs: { outerPopupRef, innerPopupRef } } = this;
...
describe('Trigger Render', () => {
  describe('Actions', () => {
    ...
    it('Nest Actions', async () => {
      ...
      // 异步渲染
      await waitTime(20);
      expect($(outerRef.portal.$el)
        .find('.a-b')
        .css('display')).not.toBe('none');
      expect($(innerRef.portal.$el)
        .find('.inner-a-b')
        .css('display')).not.toBe('none');
    ...
    it('change trigger slot', async () => {
      const mockClick = jest.fn();
      const wrapper = shallowMount(SlotTrigger, {
        sync: false,
        methods: {
          onClick: mockClick,
        },
