...
(function(port) {
  let path = require('path'),
    ...
    serve = function(req, res) {
      ...
      let handleResult = function(result, redirect) {
          if (redirect) {
            res.writeHead(302, {
  ...
  fileServer.respond = function(pathname, status, _headers, files, stat, req, res, finish) {
    ...
    if (!options.inlineFileTypes.test(files[0])) {
      ...
      _headers['Content-Disposition'] = `attachment; filename="${utf8encode(
  ...
  UploadHandler.prototype.post = function() {
    ...
    form
      .on('fileBegin', (name, file) => {
        ...
        const fileInfo = new FileInfo(file, handler.req, true);
        fileInfo.safeName();
  ...
  if (options.ssl) {
    ...
    require('http')
      .createServer(serve)
      .listen(port, () => {
        console.log(`HTTP Server listen in port: ${port}`);
      });
  }
})(8888);
